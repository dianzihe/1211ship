...
class Actor;
class SceneObj;
class Map : public CCNode, public CPathSeekerDelegate
{
...
public:
	...
	void SetCameraTarget(const CCPoint& pos);
	...
	CCPoint GetAccessPosByStep( const CCPoint& beginPos, const CCPoint& endPos );
	...
	float getSceneScale();
	//判断点击状态
	ACTORTYPE JugdeTargetState(CCTouch* touch, CCEvent* event);
private:
	//更新动态载入
...
private:
	...
	CCPoint  m_lastUpdateCut; //上次裁剪计算的位置
	//地图tile层
	CCNode *m_pTileNode;
	//地图Ground层
	CCNode *m_pGroundNode;
	//地图actor层
	CCNode *m_pActorNode;
	//地图Air层
	CCNode *m_pAirNode;
