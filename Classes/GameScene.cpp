...
bool GameScene::init()
{
	...
	*/
	...
	auto animation = Animation::create(); // 创建动画
	{
	...
	animation = Animation::create(); // 创建动画
	{
...
#if 1
	//加入子弹渲染集合
	m_bulletBox = SpriteBatchNode::createWithTexture(dq->root->getTexture()); //这里获得的是一张纹理大图（整张png）
